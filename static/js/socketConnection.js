...
function DataHandler(info){
	//tcp Listener 등록
	var hosts = startWindow.contentWindow.hosts;
	var NvHTTP = startWindow.contentWindow.NvHTTP;
	var pairTo = startWindow.contentWindow.pairTo;
	var saveHosts = startWindow.contentWindow.saveHosts;
	var beginBackgroundPollingOfHost = startWindow.contentWindow.beginBackgroundPollingOfHost;
	var addHostToGrid = startWindow.contentWindow.addHostToGrid;
	var snackbarLog = startWindow.contentWindow.snackbarLog;
	var startGame = startWindow.contentWindow.startGame;
	var stopGame = startWindow.contentWindow.stopGame;
	var myUniqueid = startWindow.contentWindow.myUniqueid;
	var document = startWindow.contentWindow.document;
	var api = startWindow.contentWindow.api;
	...
	switch(msgObject.header.command){	//msgOBject의 command값에 따라 다른 작업을 수행
		...
		pairTo(_nvhttpHost, function() {		//Pairing 작업을 수행할 PairTo함수를 호출, 두번째 인자는 pairing성공 시 호출할 콜백, 세번째 인자는 실패 시 호출
            ...
            sendMsgtoCentralServer(msg);
			snackbarLog('pairing to ' + msgObject.body.hostIpaddress + ' failed!');
        }, "0000");
		break;
		...
		case "startGame":  //특정 호스트의 특정 게임을 실행해달라는 요청
			...
			host.pollServer(function(){
				...
				startGame(hosts[msgObject.body.hostId], msgObject.body.appId, msgObject.body.option)
				.then(()=>{
					msg.header.statusCode = 200;
					sendMsgtoCentralServer(msg);					
				})
...
function closeConnection(){
	...
	chrome.sockets.tcp.getSockets(function (sockets) {
		sockets.forEach(function (sock) {
			chrome.sockets.tcp.onReceive.removeListener(DataHandler);
...
function createConnection(){
	chrome.sockets.tcp.create({}, function(createInfo){
		...
		connectRegularly = setInterval(function(){
